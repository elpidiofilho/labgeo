...
kriga <- function(df, target_var, nrep = 10,  name_px, name_py, p = 0.75, seed = NULL) {
  ng = nrep
  if (!is.null(seed)) {
    set.seed(seed)
  }
  ...
  for (i in 1:ng){
    ...
    ddd = data.frame(predito = kr$krige_output$var1.pred, observado= select(dsel.valida@data, one_of(target_var)))
